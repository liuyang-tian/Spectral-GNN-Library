...
class DataLoader():
    def __init__(self, name):
        ...
        if name in ['cora', 'citeseer', 'pubmed']:
            root_path = '../'
            path = os.path.join(root_path, 'datasets', name)
            ...
            dataset = dataset_heterophily(
                root='../datasets/'+name+'/', name=name, transform=T.NormalizeFeatures())
        elif name in ['texas', 'cornell']:
            dataset = WebKB(root='../datasets/'+name+'/', name=name,
...
class dataset_heterophily(InMemoryDataset):
    def __init__(self, root='../datasets/', name=None,
                 p2raw=None,
                 train_percent=0.01,
        ...
        if (p2raw is not None) and os.path.isdir(p2raw):
            self.p2raw = p2raw
        elif p2raw is None:
            self.p2raw = None
        elif not os.path.isdir(p2raw):
            raise ValueError(
    ...
    def process(self):
        p2f = os.path.join(self.raw_dir, self.name)
